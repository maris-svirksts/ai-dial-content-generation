...
async def _put_image() -> Attachment:
    ...
    async with DialBucketClient(api_key=API_KEY, base_url=DIAL_URL) as client:
        ...
        await client.put_file(name=file_name, mime_type=mime_type_png, content=image_bytes)
        #  5. Get bucket ID for constructing the correct URL
        bucket_id = await client._get_bucket()
        #  6. Return Attachment object with title (file name), url and type (mime type)
        #     URL format: /v1/files/{bucket_id}/{file_name} (without base URL, API will resolve it)
        return Attachment(title=file_name, url=f"files/{bucket_id}/{file_name}", type=mime_type_png)
